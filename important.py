...
df=pd.read_excel('C:/Users/naman/Downloads/Base_sondage_maraichage (1).xlsx', index_col="Identifiant", na_values=['NA'])
...
st.write("""
This dataset contains agricultural information featuring various fields related to farming or horticulture. It includes details about different crops grown, their cultivation methods, field sizes, irrigation practices, and the use of shelters or greenhouses.

Each entry in the dataset represents a specific agricultural record:

Identifier: Unique ID for each record.
Crop Group: Categorization of crops being cultivated.
Production Method: Specifies how crops are grown (e.g., main or in succession).
Crop Type: Specific crop being cultivated (e.g., Courgette, Tomato, Potato).
Field Size: Area of the cultivation field.
Irrigation: Indicates if irrigation is used (Yes/No).
Irrigation Method: Describes how irrigation is applied (e.g., Gravity, Localized).
Shelter Use: Specifies whether a greenhouse or shelter is utilized.
This data can be valuable for agricultural analysis, aiding in understanding crop preferences, cultivation techniques, field sizes, and irrigation methods. It's useful for making informed decisions related to farming practices and crop management strategies.
**
""")
